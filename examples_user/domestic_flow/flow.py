...
# 선점 스크리너 기준
PREEMPT_VOL_RATIO   = 3.0          # 오늘 거래량 / 20일 평균 최소 배수
PREEMPT_PRICE_RANGE = 5.0          # 주가 등락률 ±% 이내 (주가 아직 안 터짐)
PREEMPT_DRAWDOWN    = 20.0         # 최근 20일 고점 대비 낙폭 최소 %
...
def _check_preempt(price_data: list[dict]) -> Optional[dict]:
    """
    선점 스크리너 조건:
      ① 오늘 거래량 > 20일 평균 × 3배 (매집 신호)
      ② 오늘 등락률 ±5% 이내 (주가 아직 안 터짐)
      ③ 최근 20일 고점 대비 현재가 -20% 이상 낙폭 (낙폭과대)
    """
    ...
    # ① 거래량 배수 (전일~21일 전, 최근 20거래일 평균)
    hist = price_data[1:21]
    avg_vol = sum(d['거래량'] for d in hist) / len(hist)
    if avg_vol == 0:
        return None
    vol_ratio = today['거래량'] / avg_vol
    if vol_ratio < PREEMPT_VOL_RATIO:
        return None
...
def format_preempt_message(rows: list[dict], market: str) -> str:
    desc = (
        f'거래량 {PREEMPT_VOL_RATIO:.0f}배↑ + 등락률 ±{PREEMPT_PRICE_RANGE:.0f}% + '
        f'20일 고점 대비 -{PREEMPT_DRAWDOWN:.0f}%↓'
    )
    if not rows:
        return (
            f'🎯 <b>{market} 선점 후보</b>\n\n'
    ...
    for i, r in enumerate(rows, 1):
        ...
        lines.append(
            ...
            f'   {r["현재가"]:,}원 {rate_str}\n'
            f'   거래량 <b>{r["거래량배수"]}배</b>  {_vol(r["오늘거래량"])} (평균 {_vol(r["평균거래량"])})\n'
